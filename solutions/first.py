...
def solve_first():
    ...
    with open('../files/mirror_error.csv', mode='wr', encoding='utf-8') as file:
        mirror_error_writer = writer(file, delimiter=',', lineterminator='\n')
        mirror_error_writer.writerow(fields)
        for record in good_records:
            mirror_error_writer.writerow([record.date, f'{record.lastname} {record.firstname} {record.middlename}'])
